    ...
    <div className="min-h-[calc(100vh-200px)] flex items-center justify-center px-4">
      <div className="max-w-md w-full text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-2 justify-center">
          <Link
            href={`/${locale}/account`}
            className="px-4 py-2 rounded-sm bg-accent text-white text-sm font-medium hover:opacity-90 transition-opacity"
          >
            {t('backToAccount')}
          </Link>
          <Link
            href={`/${locale}`}
            className="px-4 py-2 rounded-sm border border-border text-sm text-text-secondary hover:bg-accent/5 transition-colors"
          >
            {t('backHome')}
          </Link>
        </div>
