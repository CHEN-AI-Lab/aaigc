...
function CalcPanel() {
  ...
  const btn = (label: string, base: string, onClick: () => void) => (
    <button key={label} onClick={onClick} className={`p-3 text-sm rounded-lg font-medium transition-colors active:scale-90 duration-100 ${base}`}>{label}</button>
  )
  const sbtn = (l: string, b: string, onClick: () => void) => (
    <button key={l} onClick={onClick} className={`p-2 text-xs rounded-lg font-medium transition-colors active:scale-90 duration-100 ${b}`}>{l}</button>
  )

  return (
    <div className="mx-auto" style={{ maxWidth: '260px' }}>
      <div className="bg-surface border border-[rgba(127,99,21,0.15)] rounded-lg p-3 mb-3 min-h-[4rem]">
...
function CurrencyCalc() {
  ...
  return (
    <div className="max-w-md mx-auto space-y-3">
      <div className="bg-surface rounded-lg border border-[rgba(127,99,21,0.1)] p-4">
        <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-end">
          <div>
            <label className="block text-xs text-text-secondary mb-1">{t('currencyAmount')}</label>
            <input value={amount} onChange={e => setAmount(e.target.value)} maxLength={15} className="w-full p-2 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary" />
          </div>
          <div />
          <div />
          <div>
            <select value={from} onChange={e => setFrom(e.target.value)} className="w-full p-2 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary">
          ...
          <div>
            <select value={to} onChange={e => setTo(e.target.value)} className="w-full p-2 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary">
              {Object.keys(RATES).map(c => <option key={c} value={c}>{c} ({t(`currency${c}`)})</option>)}
            </select>
          </div>
        </div>
      ...
      {result !== null && (
        <div className="bg-surface rounded-lg border border-[rgba(127,99,21,0.1)] p-3">
          ...
          <div className="grid grid-cols-2 gap-1 text-xs">
            {Object.keys(RATES).filter(c => c !== from).map(c => (
              <div key={c} className="flex justify-between p-1.5 bg-card rounded-lg">
      ...
      <div className="bg-surface rounded-lg border border-[rgba(127,99,21,0.1)] p-4">
        <div className="flex gap-2 items-end mb-3">
          <div className="flex-1">
            ...
            <input value={val} onChange={e => setVal(e.target.value)} maxLength={15} className="w-full p-2 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary" />
          </div>
          <div className="w-24">
        ...
        {!isNaN(n) && uFrom && (
          <div className="space-y-0.5">
            {units.filter(u => u.label !== from).map(u => (
              <div key={u.label} className="flex justify-between p-1.5 bg-card rounded-lg text-xs">
    ...
    <div className="max-w-md mx-auto space-y-3">
      <div className="bg-surface rounded-lg border border-[rgba(127,99,21,0.1)] p-4">
        <div className="flex gap-2 items-end mb-3">
          ...
          <div className="w-24">
            <label className="block text-xs text-text-secondary mb-1">{t('calcUnit')}</label>
            <select value={unit} onChange={e => setUnit(e.target.value as 'C' | 'F' | 'K')} className="w-full p-2 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary">
        ...
        {c !== null && (
          <div className="space-y-2">
            <div className="flex justify-between p-2 bg-card rounded-lg text-sm">
    ...
    <div className="max-w-md mx-auto space-y-3">
      <div className="bg-surface rounded-lg border border-[rgba(127,99,21,0.1)] p-4">
        <div className="flex gap-1 mb-3">
          <button onClick={() => setUnit('metric')} className={`px-3 py-1 text-xs rounded-lg ${unit === 'metric' ? 'bg-accent text-white' : 'bg-card text-text-secondary'}`}>{t('calcMetric')}</button>
          <button onClick={() => setUnit('imperial')} className={`px-3 py-1 text-xs rounded-lg ${unit === 'imperial' ? 'bg-accent text-white' : 'bg-card text-text-secondary'}`}>{t('calcImperial')}</button>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div>
            <label className="block text-xs text-text-secondary mb-1">{t('calcHeight')} ({unit === 'metric' ? 'cm' : 'in'})</label>
            <input value={h} onChange={e => setH(e.target.value)} maxLength={5} className="w-full p-2 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary" />
          </div>
          <div>
            <label className="block text-xs text-text-secondary mb-1">{t('calcWeight')} ({unit === 'metric' ? 'kg' : 'lb'})</label>
            <input value={w} onChange={e => setW(e.target.value)} maxLength={5} className="w-full p-2 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary" />
          </div>
          <div>
            <label className="block text-xs text-text-secondary mb-1">{t('calcAge')}</label>
            <input value={age} onChange={e => setAge(e.target.value)} className="w-full p-2 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary" />
          </div>
        </div>
        <div className="flex gap-1 mt-2">
          <button onClick={() => setGender('male')} className={`flex-1 p-2 text-xs rounded-lg ${gender === 'male' ? 'bg-accent text-white' : 'bg-card text-text-secondary border border-[rgba(127,99,21,0.1)]'}`}>{t('calcMale')}</button>
          <button onClick={() => setGender('female')} className={`flex-1 p-2 text-xs rounded-lg ${gender === 'female' ? 'bg-accent text-white' : 'bg-card text-text-secondary border border-[rgba(127,99,21,0.1)]'}`}>{t('calcFemale')}</button>
        </div>
        {bmi && (
...
function TaxCalc() {
  ...
  const inputClass = "w-full p-2 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary"
  const labelClass = "block text-xs text-text-secondary mb-1"

  return (
    <div className="max-w-md mx-auto space-y-3">
      <div className="bg-surface border border-[rgba(127,99,21,0.15)] rounded-lg p-3 text-xs text-text-secondary leading-relaxed">
        {t('chinaOnlyNote')}
      </div>
      <div className="bg-surface rounded-lg border border-[rgba(127,99,21,0.1)] p-4">
        {/* Basic salary inputs */}
        ...
        {s > 0 && (
          <div className="space-y-2">
            <div className="flex justify-between text-xs p-1.5 bg-card rounded-lg">
        ...
        {s > 0 && (
          <div className="mt-3 space-y-2">
            {salaryResult && (
              <>
                <div className="flex justify-between text-sm p-2 bg-card rounded-lg">
            ...
            {(salaryResult || bonusResult) && (
              <div className="flex justify-between text-sm p-2 bg-card rounded-lg border-t border-[rgba(127,99,21,0.1)]">
            ...
            <div className="flex justify-between text-sm p-2 bg-green-50 rounded-lg border border-green-200">
        ...
        <div className="bg-surface rounded-lg border border-[rgba(127,99,21,0.1)] p-3">
          ...
          <div className="space-y-0.5">
            {yearlyBrackets.map(b => (
              <div key={b.low} className={`flex justify-between text-xs p-1.5 rounded-lg ${b.label === salaryResult?.bracket ? 'bg-accent/10 text-accent font-medium' : 'text-text-secondary'}`}>
                <span>{t(b.label)}</span>
...
function MortgageCalc() {
  ...
  const btnClass = (active: boolean) =>
    `px-3 py-1 text-xs rounded-lg ${active ? 'bg-accent text-white' : 'bg-card text-text-secondary'}`
  const inputClass = "w-full p-2 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary"
  const labelClass = "block text-xs text-text-secondary mb-1"
        ...
        {result && result.monthly > 0 && (
          <div className="mt-3 pt-3 border-t border-[rgba(127,99,21,0.1)] space-y-2">
            ...
            {type === 'mixed' && 'commercial' in result && 'fund' in result && (
              <div className="p-2 bg-card rounded-lg text-xs space-y-1">
              ...
              return (
                <div key={year} className="mb-1">
                  <button
                    ...
                    className="w-full flex items-center justify-between p-2 bg-card rounded-lg text-xs hover:bg-accent/5 transition-colors"
                  >
                  ...
                  {isExpanded && (
                    <div className="mt-1 space-y-0.5 ml-2">
                      ...
                      {yearMonths.map(m => (
                        <div key={m.month} className="flex items-center text-[11px] px-2 py-1 bg-card rounded-lg">
...
function ChineseNumCalc() {
  ...
  return (
    <div className="max-w-md mx-auto space-y-3">
      ...
      <div className="bg-surface rounded-lg border border-[rgba(127,99,21,0.1)] p-4">
        <div className="flex gap-1 mb-3">
          <button onClick={() => setMode('chinese')} className={`px-3 py-1.5 text-xs rounded-lg ${mode === 'chinese' ? 'bg-accent text-white' : 'bg-card text-text-secondary border border-[rgba(127,99,21,0.1)]'}`}>{t('calcChineseMode')}</button>
          <button onClick={() => setMode('roman')} className={`px-3 py-1.5 text-xs rounded-lg ${mode === 'roman' ? 'bg-accent text-white' : 'bg-card text-text-secondary border border-[rgba(127,99,21,0.1)]'}`}>{t('calcRomanMode')}</button>
        </div>
        <label className="block text-xs text-text-secondary mb-1">{t('calcInputNumber')}</label>
        <input value={num} onChange={e => setNum(e.target.value)} placeholder="12345.67" className="w-full p-2 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary" />
        {result && (
          <div className="mt-3 p-3 bg-card rounded-lg border border-[rgba(127,99,21,0.08)]">
...
function TimeCalc() {
  ...
  return (
    <div className="max-w-md mx-auto space-y-3">
      <div className="bg-surface rounded-lg border border-[rgba(127,99,21,0.1)] p-4">
        <div className="bg-surface border border-[rgba(127,99,21,0.15)] rounded-lg p-3 text-xs space-y-1">
...
function TitleCalc() {
  ...
  return (
      <div className="max-w-lg mx-auto space-y-3">
        <div className="bg-surface border border-[rgba(127,99,21,0.15)] rounded-lg p-3 text-xs text-text-secondary leading-relaxed space-y-1">
          {t('calcTitleNote')}
          <p>{t('chinaOnlyNote')}</p>
        </div>
        <div className="bg-surface rounded-lg border border-[rgba(127,99,21,0.1)] p-4">
        ...
        {selected1 && selected3 && (
          <div className={`mt-3 p-3 bg-card rounded-lg border text-center ${
          ...
          {commonRelations.map(k => {
            ...
            return (
              <button key={k} onClick={() => {
                ...
                className="text-left text-xs p-1.5 bg-card rounded-lg hover:bg-accent/5 text-text-primary">
                {k} → {relations[k]}
              </button>
            )
          })}
      ...
      <div className="bg-surface rounded-lg border border-[rgba(127,99,21,0.1)] p-5">
        <div className="flex gap-2 mb-3">
          <input value={input} onChange={e => setInput(e.target.value)} placeholder="{t('calcInputNumber')}"
            className="flex-1 p-3 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm font-mono text-text-primary focus:outline-none focus:border-accent/30" />
          <select value={fromBase} onChange={e => { setInput(''); setFromBase(parseInt(e.target.value)) }}
            className="p-3 bg-card border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary focus:outline-none">
        ...
        {results.length > 0 && (
          <div className="space-y-1.5">
            {results.map(r => (
              <div key={r.base} className="flex items-center gap-3 p-3 bg-surface rounded-lg border border-[rgba(127,99,21,0.08)]">
