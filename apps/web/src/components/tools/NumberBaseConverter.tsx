        ...
        <div className="space-y-3">
          <div>
            ...
            <input value={input} onChange={e => setInput(e.target.value)} placeholder="e.g. 255, FF, 11111111"
              className="w-full p-3 bg-surface border border-border rounded-sm text-sm font-mono text-text-primary placeholder:text-text-secondary/50 focus:outline-none focus:border-accent/30" />
          </div>
          <div className="flex items-end gap-3">
            <div className="flex-1 sm:max-w-[200px]">
              ...
              <select value={fromBase} onChange={e => setFromBase(parseInt(e.target.value))}
                className="w-full p-3 bg-surface border border-border rounded-sm text-sm text-text-primary focus:outline-none">
                {BASES.map(({ base, label }) => (
                  <option key={base} value={base}>{label} ({t(`base${base}`)})</option>
                ))}
              </select>
            </div>
            <button onClick={convert} className="px-8 py-3 bg-accent text-white text-sm font-medium rounded-sm hover:opacity-90 transition-opacity shrink-0">
              {t('convert')}
            </button>
          </div>
        </div>
