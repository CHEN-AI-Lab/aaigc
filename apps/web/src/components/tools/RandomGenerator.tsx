    ...
    <div className="mt-6 space-y-4">
      ...
      <div className="flex gap-2">
        <button onClick={() => setMode('number')}
          className={`px-4 py-2 text-sm rounded-sm transition-colors ${mode === 'number' ? 'bg-accent text-white' : 'bg-surface text-text-primary border border-border'}`}>随机数字</button>
        <button onClick={() => setMode('string')}
          className={`px-4 py-2 text-sm rounded-sm transition-colors ${mode === 'string' ? 'bg-accent text-white' : 'bg-surface text-text-primary border border-border'}`}>随机字符串</button>
        <button onClick={() => setMode('color')}
          className={`px-4 py-2 text-sm rounded-sm transition-colors ${mode === 'color' ? 'bg-accent text-white' : 'bg-surface text-text-primary border border-border'}`}>随机颜色</button>
      </div>
