    ...
    <div className="mt-6 space-y-4">
      ...
      {uuid && (
        <div className="relative">
          <textarea readOnly value={uuid} className="w-full h-36 p-3 bg-surface border border-[rgba(127,99,21,0.15)] rounded-lg text-sm font-mono text-text-primary resize-none" />
          <button onClick={() => navigator.clipboard.writeText(uuid)} className="absolute top-2 right-6 text-xs px-2 py-1 bg-accent text-white rounded-lg hover:opacity-90 min-w-[4.5rem] text-center">{t('copy')}</button>
        </div>
      )}
    </div>
  )
