  ...
  return (
    <div className="mt-6 space-y-6">
        ...
        <div className="flex-1 space-y-2">
          {[
            ...
            <div key={label} className="flex items-center gap-2">
              <span className="text-xs font-mono text-text-secondary w-10">{label}</span>
              <code className="text-sm text-text-primary font-mono flex-1">{value}</code>
              <button onClick={() => navigator.clipboard.writeText(value)} className="text-xs px-2 py-0.5 bg-surface text-text-secondary rounded-sm hover:text-accent transition-colors">
                {locale === 'en' ? 'Copy' : '复制'}
              </button>
            </div>
          ))}
      ...
      <div className="grid grid-cols-10 gap-1">
        {['#fa520f','#ffa110','#ffd900','#ff8a00','#fb6424','#1f1f1f','#767d88','#fffaeb','#fff0c2','#3ecf8e'].map(c => (
