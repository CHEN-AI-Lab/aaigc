...
export default function ColorPicker() {
  ...
  return (
    <div className="mt-6 space-y-6">
      <div className="flex items-center gap-6 p-4 bg-surface rounded-lg border border-[rgba(127,99,21,0.15)]">
        <div className="flex flex-col items-center gap-2">
          <input type="color" value={color} onChange={e => setColor(e.target.value)} className="w-16 h-16 rounded-lg cursor-pointer border border-[rgba(127,99,21,0.15)]" />
          ...
          {values.map(({ label, value }, i) => (
            <div key={label} className="flex items-center gap-2">
              ...
              <button
                ...
                className={`text-xs px-2 py-0.5 bg-accent text-white rounded-lg transition-all min-w-[4.5rem] text-center ${animIndex === i ? 'scale-110 opacity-70' : 'hover:opacity-90'}`}
              >
                {t('copy')}
              </button>
            </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-10 sm:grid-cols-15 gap-1.5">
          {presets.map(c => (
            <button key={c} onClick={() => setColor(c)} className="h-8 rounded-lg border border-[rgba(127,99,21,0.1)] cursor-pointer hover:scale-110 transition-transform" style={{ backgroundColor: c }} title={c} />
          ))}
        </div>
      </div>
    </div>
