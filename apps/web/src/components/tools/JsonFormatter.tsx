      ...
      {output && (
        <div className="relative">
          <textarea
            readOnly
            value={output}
            className="w-full h-64 p-3 pr-24 bg-surface border border-[rgba(127,99,21,0.15)] rounded-sm text-sm font-mono text-text-primary resize-none"
          />
          <div className="absolute top-2 right-2 flex items-center gap-1.5">
            <button
              onClick={handleCopy}
              className="relative text-xs px-2.5 py-1.5 bg-dark text-white rounded-sm hover:opacity-90 transition-opacity"
            >
            ...
            {downloadUrl && (
              <a
                ...
                className="text-xs px-2.5 py-1.5 bg-dark text-white rounded-sm hover:opacity-90 transition-opacity"
              >
