  ...
  return (
    <div className="mt-6 space-y-4">
      <textarea
        ...
        className="w-full h-36 p-3 bg-surface border border-[rgba(127,99,21,0.15)] rounded-lg text-sm font-mono text-text-primary placeholder:text-text-secondary/50 resize-none focus:outline-none focus:border-accent/30"
      />
      <div className="flex gap-2 flex-wrap items-center">
        ...
        <button onClick={validate} className="px-4 py-2 bg-surface text-text-primary text-sm rounded-lg hover:opacity-90 border border-[rgba(127,99,21,0.15)]">{t('validate')}</button>
        <button onClick={minify} className="px-4 py-2 bg-surface text-text-primary text-sm rounded-lg hover:opacity-90 border border-[rgba(127,99,21,0.15)]">{t('minify')}</button>
        {downloadUrl && (
          <a
            href={downloadUrl}
      ...
      {output && (
        <div className="relative">
          ...
          <div
            ref={scrollRef}
            className="max-h-[60vh] overflow-auto bg-surface border border-[rgba(127,99,21,0.15)] rounded-lg pr-10"
          >
          ...
          <button
            onClick={handleCopy}
            className={`absolute top-2 right-8 text-xs px-2.5 py-1.5 rounded-lg transition-all duration-200 min-w-[4.5rem] text-center ${
              copied
