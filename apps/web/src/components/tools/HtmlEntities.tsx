  ...
  return (
    <div className="mt-6 space-y-4">
      <div className="flex gap-2">
        <button onClick={() => setMode('escape')} className={`px-4 py-1.5 text-sm rounded-lg transition-colors ${mode === 'escape' ? 'bg-accent text-white' : 'bg-surface text-text-primary'}`}>{t('escape')}</button>
        <button onClick={() => setMode('unescape')} className={`px-4 py-1.5 text-sm rounded-lg transition-colors ${mode === 'unescape' ? 'bg-accent text-white' : 'bg-surface text-text-primary'}`}>{t('unescape')}</button>
      </div>
      <textarea value={input} onChange={e => setInput(e.target.value)} placeholder={t('enterHtml')} className="w-full h-28 p-3 bg-surface border border-[rgba(127,99,21,0.15)] rounded-lg text-sm text-text-primary placeholder:text-text-secondary/50 resize-none focus:outline-none focus:border-accent/30" />
