...
export default function YamlJsonConverter() {
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      <textarea value={input} onChange={e => setInput(e.target.value)} placeholder={mode === 'yaml2json' ? 'key: value' : '{"key": "value"}'} className="w-full h-36 p-3 bg-surface border border-border rounded-sm text-sm font-mono text-text-primary placeholder:text-text-secondary/50 resize-none focus:outline-none focus:border-accent/30" />
      ...
      {output && (
        <div className="relative">
          <textarea readOnly value={output} className="w-full h-36 p-3 bg-surface border border-border rounded-sm text-sm font-mono text-text-primary resize-none" />
