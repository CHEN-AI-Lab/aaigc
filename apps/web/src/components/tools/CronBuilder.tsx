      ...
      <div className="flex gap-2 flex-wrap">
        {PRESETS.map(p => (
          <button key={p.expr} onClick={() => preset(p.expr)}
            className={`px-3 py-1.5 text-xs rounded-md transition-colors ${expression === p.expr ? 'bg-accent text-white' : 'bg-surface text-text-primary border border-[rgba(127,99,21,0.15)]'}`}
            title={t(p.desc)}>{t(p.label)}</button>
        ))}
      </div>

      {dangerous && (
        <div className="p-3 bg-red-50 border border-red-200 rounded-md text-xs text-red-700 leading-relaxed">
          <p className="font-semibold mb-1">⚠️ {t('cronWarning')}</p>
          <button onClick={() => { setMinute('0'); setHour('0') }}
            className="mt-1 px-3 py-1 bg-red-500 text-white rounded-md hover:opacity-90">{t('cronFix')}</button>
        </div>
      )}

      <div className="grid grid-cols-5 gap-3">
        <div><label className="text-xs text-text-secondary block mb-1">{t('cronMinute')}</label>
          <select value={minute} onChange={e => setMinute(e.target.value)} className="w-full p-2 bg-surface border border-[rgba(127,99,21,0.15)] rounded-md text-xs text-text-primary">
        ...
        <div><label className="text-xs text-text-secondary block mb-1">{t('cronHour')}</label>
          <select value={hour} onChange={e => setHour(e.target.value)} className="w-full p-2 bg-surface border border-[rgba(127,99,21,0.15)] rounded-md text-xs text-text-primary">
            <option value="*">{t('cronEvery')}</option>
        ...
        <div><label className="text-xs text-text-secondary block mb-1">{t('cronDay')}</label>
          <select value={day} onChange={e => setDay(e.target.value)} className="w-full p-2 bg-surface border border-[rgba(127,99,21,0.15)] rounded-md text-xs text-text-primary">
            <option value="*">{t('cronEvery')}</option>
        ...
        <div><label className="text-xs text-text-secondary block mb-1">{t('cronMonth')}</label>
          <select value={month} onChange={e => setMonth(e.target.value)} className="w-full p-2 bg-surface border border-[rgba(127,99,21,0.15)] rounded-md text-xs text-text-primary">
            <option value="*">{t('cronEvery')}</option>
        ...
        <div><label className="text-xs text-text-secondary block mb-1">{t('cronDow')}</label>
          <select value={dow} onChange={e => setDow(e.target.value)} className="w-full p-2 bg-surface border border-[rgba(127,99,21,0.15)] rounded-md text-xs text-text-primary">
            <option value="*">{t('cronEvery')}</option>
            {DOW.map((d, i) => <option key={i} value={i}>{t('cronWeekPrefix')}{t(d)}</option>)}
          </select></div>
      </div>

      <div className="p-4 bg-surface border border-[rgba(127,99,21,0.15)] rounded-md text-center">
        ...
        <div className="mt-2 flex items-center justify-center gap-2">
          <button onClick={() => { navigator.clipboard.writeText(expression); setCopied(true); setTimeout(() => setCopied(false), 2000) }}
            className="px-4 py-1.5 bg-accent text-white text-xs rounded-md hover:opacity-90">{copied ? t('cronCopied') : t('cronCopy')}</button>
        </div>
