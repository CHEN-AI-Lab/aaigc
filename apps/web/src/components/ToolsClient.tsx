    ...
    <div className="max-w-6xl mx-auto px-6 py-16">
      ...
      <div className="mb-8">
        <input
          ...
          className="w-full max-w-md p-3 bg-card border border-[rgba(127,99,21,0.15)] rounded-md text-sm text-text-primary placeholder:text-text-secondary/50 focus:outline-none focus:border-accent/30"
        />
      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {filtered.map((tool) => (
                <Link key={tool.id} href={`/tools/${tool.id}`}
                  className="block bg-card rounded-md p-4 shadow-warm-sm hover:shadow-warm transition-shadow border border-[rgba(127,99,21,0.05)]">
