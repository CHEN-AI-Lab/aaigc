...
| 阶段 | 总大小 | 备注 |
|---|---|---|
| T05.2.2 spike（**仅 base64 一个工具**） | 376 KB | Taro runtime 132 KB 为固定成本，约占 2MB 的 6.6% |
| 工具全量接入（≥22 个） | **待补录** | 单工具编译后约 4 KB 量级，粗估 150–200 KB |

> ⚠️ **376 KB 是 1 个工具的数据，不是 22+ 个。** 不能用它外推全量余量。
> 每接入一批工具必须重新测量，并更新本表。
